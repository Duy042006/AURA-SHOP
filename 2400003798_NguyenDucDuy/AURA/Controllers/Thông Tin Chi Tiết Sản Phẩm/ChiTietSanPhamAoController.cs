...
{
    public class ChiTietSanPhamAoController : Controller
    {
        AURA_DBContext db = new AURA_DBContext();

        // ================================
        // TRANG CHI TIẾT SẢN PHẨM ÁO
        // ================================
        public ActionResult Index(int? id)
        {
            // ✅ Nếu không có id → quay về trang Quần
            if (id == null)
                return RedirectToAction("Index", "Ao");

            // ✅ Lấy sản phẩm QUẦN từ View_SanPhamTong
            var sanPham = db.View_SanPhamTong
                            .FirstOrDefault(x => x.MaSP == id && x.LoaiSP == "Áo");

            if (sanPham == null)
                return HttpNotFound();

            // ✅ Lấy chi tiết sản phẩm QUẦN
            var chiTiet = db.ChiTietSanPhamAos
